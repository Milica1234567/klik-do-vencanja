...
>(function InviteAmbientMusic({ music, unlocked: unlockedProp = false }, ref) {
  ...
  return (
    <div className="invitation4-ambient">
      {youtubeId ? (
        <iframe
          ref={iframeRef}
          className="invitation4-ambient__frame"
          title={title}
      ...
      <button
        type="button"
        className={`invitation4-ambient__btn${isAudible ? " is-on" : ""}`}
        onClick={toggle}
        aria-pressed={isAudible}
        aria-label={isAudible ? "Isključi muziku" : "Uključi muziku"}
      >
        <span className="invitation4-ambient__icon" aria-hidden="true">
          {isAudible ? (
